...
namespace path_finding {
    ...
    Map::Map(const vec2 &top_left_corner, const vec2 &num_pixels_per_side,
        ...
        for (size_t i = 0; i < num_pixels_per_side.y; i++) {
            ...
            graph_.emplace_back(temp);
        }

        //sets start and end points
        graph_[start_point_coords_.y][start_point_coords_.x] = 's';
        graph_[end_point_coords_.y][end_point_coords_.x] = 'e';
        //creates the solved drawing
        solved_graph_ = graph_;
        DrawPath();
    }

    void Map::DrawHelper() const {
        if (show_path_)
            Draw(solved_graph_);
        else
            Draw(graph_);
    ...
    void Map::Draw(std::vector<std::vector<char>> array) const {
    ...
    void Map::Brush(const vec2 &brush_screen_coords, bool draw_point, bool start_point) {
        ...
        if (draw_point) {
            ...
            if (start_point) {
                graph_[start_point_coords_.y][start_point_coords_.x] = 'w';
                graph_[brush_sketchpad_coords.y][brush_sketchpad_coords.x] = 's';
                start_point_coords_ = vec2((size_t) brush_sketchpad_coords.x, (size_t) brush_sketchpad_coords.y);
            } else {
                graph_[end_point_coords_.y][end_point_coords_.x] = 'w';
                graph_[brush_sketchpad_coords.y][brush_sketchpad_coords.x] = 'e';
            ...
            for (size_t row = 0; row < num_pixels_per_side_.y; ++row) {
                for (size_t col = 0; col < num_pixels_per_side_.x; ++col) {
                    ...
                    if (glm::distance(brush_sketchpad_coords, pixel_center) <=
                        brush_radius) {
                        if ((vec2(row, col) != vec2(size_t(start_point_coords_.y), size_t(start_point_coords_.x))) &&
                            ((vec2(row, col) != vec2(size_t(end_point_coords_.y), size_t(end_point_coords_.x)))))
                            graph_[row][col] = graph_[row][col] == '#' ? 'w' : '#';
                    }
                }
            }
        }
    ...
    std::vector<std::vector<char>> Map::GetGraph() {
        return graph_;
    }

    void Map::DrawPath() {
        //runs AStar Algorithm on this graph
        AStar astar(graph_, start_point_coords_, end_point_coords_);
        astar.ShortestPath();
        solved_graph_ = astar.GetGraph();
    }
    ...
    std::ostream &operator<<(std::ostream &os, Map &map) {
        os << map.start_point_coords_.x << " " << map.start_point_coords_.y << std::endl;
        os << map.end_point_coords_.x << " " << map.end_point_coords_.y << std::endl;
        os << map.num_pixels_per_side_.x << " " << map.num_pixels_per_side_.y << std::endl;

        for (auto &i: map.graph_) {
            for (char &j: i) {
                os << std::to_string(j) + ' ';
            }
            os << "\n";
    ...
    std::istream &operator>>(std::istream &is, Map &map) {
        ...
        map.start_point_coords_ = vec2(x, y);

        getline(is, line);
        x = std::stoi(line, &sz);
        line = line.substr(sz);
        y = std::stoi(line, &sz);
        map.end_point_coords_ = vec2(x, y);

        getline(is, line);
        x = std::stoi(line, &sz);
        line = line.substr(sz);
        y = std::stoi(line, &sz);
        map.pixel_side_length_ = map.pixel_side_length_ * map.num_pixels_per_side_.y / y;
        map.num_pixels_per_side_ = vec2(x, y);
        ...
        map.graph_ = image;
        map.solved_graph_ = image;
        map.show_path_ = false;
        map.DrawPath();
