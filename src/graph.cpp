...
namespace path_finding {
    ...
    const vector<Vertex> & Graph::getAdjacent(Vertex source) const {
    ...
    const Vertex & Graph::getStartingVertex() const {
    ...
    const Edge & Graph::getEdge(Vertex source, Vertex destination) const {
    ...
    const string &Graph::getEdgeLabel(Vertex source, Vertex destination) const {
        if (assertEdgeExists(source, destination, __func__) == false)
            return InvalidLabel;
